 ...
 //var_dump($i);
            ...
            if ($this->IsInLexicon($wordBeingTested)) {
                ...
                if ("kind" !=$words_and_emoticons[$i] && "of" != $words_and_emoticons[$i]) {
                    ...
                    if ($valence > 0) {
                        $positive_words[] = $wordBeingTested;
                    } elseif ($valence < 0) {
                        $negative_words[] = $wordBeingTested;
                    }
        ...
        $result['pos_words'] = $positive_words;
        $result['neg_words'] = $negative_words;
        
        return $this->score_valence($sentiments, $text, $result['pos_words'], $result['neg_words']);
    }
...
*/
    ...
    public function score_valence($sentiments, $text, $positive, $negative)
    {
        if ($sentiments) {
        ...
        $sentiment_dict =
            ["neg" => round($neg, 3),
             ...
             "pos_words" => $positive, // Add positive words
             "neg_words" => $negative ];
